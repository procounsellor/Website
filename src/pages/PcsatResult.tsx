...
export default function PcsatResultPage() {
  ...
  const handleSearch = async () => {
    if (!searchInput.trim()) {
      setError("Please enter a User ID or Phone Number");
      return;
    }
    ...
    try {
      ...
      if (found) {
        ...
        setUserResult(null);
        setError("User ID or Phone Number not found in results.");
      }
    } catch (err) {
        ...
        <div className="rounded-2xl bg-white border border-[#E4E8EC] overflow-hidden">
          <div className="px-4 md:px-6 py-5 border-b border-[#E4E8EC]">
            ...
            <p className="text-(--text-muted) text-sm md:text-base mt-1">
              Enter your User ID or Phone Number to check your rank and marks.
            </p>
          </div>
          ...
          {loading && (
            <div className="px-4 md:px-6 py-8 flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-(--text-muted) text-sm">Fetching your result...</p>
            </div>
          )}

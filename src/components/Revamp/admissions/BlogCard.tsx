  ...
  return (
    ...
    >
      ...
      <div className="absolute flex flex-col w-full h-full z-10 p-3">
        <img src={imageUrl} alt={title} className="w-full h-[167px] rounded-[8px] object-cover shrink-0" />
        <h1 className="font-medium mt-2.5 text-[1rem] text-(--text-main) line-clamp-3">{title}</h1>
        <div className="mt-auto mb-[6px]">
          <p className="text-(--text-muted) font-medium text-[0.875rem]">By {author}</p>
          <p className="text-(--text-muted) font-medium text-[0.75rem]">{readTime}</p>
        </div>
      </div>

...
import { Loader2, ChevronRight, Users, Star } from 'lucide-react';
...
export default function RevampCounselorCoursesCard({ counsellorId, userRole = "user" }: CounselorCoursesCardProps) {
  ...
  if (isLoading) {
    return (
      <div className="w-full max-w-[580px] bg-white rounded-[16px] border border-[#EFEFEF] p-8 flex justify-center items-center">
  ...
  return (
    <div className="w-full max-w-[580px] bg-white rounded-[16px] border border-[#EFEFEF] p-[12px] font-poppins shadow-sm">
      
      {/* Header */}
      <div className="flex justify-between items-center px-[4px]">
        <h2 className="text-[20px] font-semibold text-[#0E1629] leading-none">
          Courses
        </h2>
        <button 
          onClick={() => navigate('/courses/course-listing')}
          className="flex items-center gap-[2px] text-[14px] font-semibold text-[#3D3D3D] leading-none cursor-pointer hover:text-[#2F43F2] transition-colors"
        >
          See All <ChevronRight className="w-[16px] h-[16px]" />
        </button>
      </div>
      ...
      <div className="mt-[20px] flex flex-col gap-[12px]">
        {displayedCourses.map((course: any) => (
          <div 
            ...
            className="w-full h-[180px] bg-[#F5F5F5] rounded-[12px] p-[12px] flex gap-[16px] cursor-pointer hover:bg-gray-200 transition-colors"
          >
            <img 
              src={course.courseThumbnailUrl || '/placeholder-course.jpg'} 
              alt={course.courseName}
              className="w-[184px] h-[155px] rounded-[9px] object-cover shrink-0"
            />
            ...
            <div className="flex flex-col flex-1 pl-[4px]">
              ...
              <div className="flex-1 flex flex-col justify-center">
                <h3 className="text-[20px] font-medium text-[#343C6A] leading-[120%] line-clamp-2">
                  {course.courseName}
                </h3>
              ...
              <div className="mt-auto flex items-center gap-[8px] pb-[4px]">
                <img src="/coin.svg" alt="Procoins" className="w-[24px] h-[24px]" />
                <div className="flex items-center gap-[4px]">
                  <span className="text-[16px] font-medium text-[#0E1629] leading-none">

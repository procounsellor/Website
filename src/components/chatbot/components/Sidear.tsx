...
// ADDED: cursor-pointer to base icon button
...
export default function Sidebar({
  ...
  // Fixed: This function is now used in the input below
                ...
                <div className="space-y-1">
                  ...
                  {chatSessions.map((chat) => (
                    <div key={chat.sessionId} className="relative">
                      {/* ADDED: cursor-pointer was already here, ensuring it stays */}
                      <div onClick={() => handleSelectChat(chat.sessionId)} className={`group w-full rounded-lg py-2 px-3 cursor-pointer transition-colors ${currentSessionId === chat.sessionId ? "bg-white/[.15]" : "hover:bg-white/[.08]"}`}>
                        <div className="flex items-center justify-between">
                          {/* FIXED: Added conditional rendering for Rename Input */}
                          {editingSessionId === chat.sessionId ? (
                            <input
                              ...
                              onClick={(e) => e.stopPropagation()} // Prevent selecting chat while clicking input

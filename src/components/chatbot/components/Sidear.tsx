...
const Tooltip = ({ text, position = "right" }: { text: string, position?: "right" | "bottom" }) => {
  const baseClasses = "absolute px-2 py-1 bg-[#2a2a2a] text-white text-xs rounded whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 z-50 border border-[#404040] shadow-xl";
...
const IconOnlyButton = ({ onClick, children, ariaLabel }: { onClick?: (e: React.MouseEvent) => void, children: React.ReactNode, ariaLabel: string }) => (
  <button
    onClick={onClick}
  ...
  >
    {children}
    <Tooltip text={ariaLabel} position="right" />
  </button>
);
...
export default function Sidebar({
  ...
  // NEW: State to track which chat is being deleted
  ...
  // MODIFIED: Instead of confirm(), set the state to open the modal
  ...
  const confirmDelete = async () => {
    if (chatToDelete && userId) {
      await deleteChatSession(chatToDelete, userId);
          ...
          <div className="bg-[#2a2a2a] rounded-2xl p-6 max-w-sm w-full border border-[#A0A0A099] shadow-2xl flex flex-col items-center text-center">
            ...
            <div className="flex gap-3 w-full">
              <button
                ...
                className="flex-1 py-2.5 bg-[#3a3a3a] hover:bg-[#4a4a4a] text-white font-medium cursor-pointer rounded-lg transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={confirmDelete}
                className="flex-1 py-2.5 bg-red-500 hover:bg-red-600 text-white cursor-pointer font-medium rounded-lg transition-colors"
              >
                Delete
              </button>
            </div>
      ...
      >
        {isSidebarOpen ? (
          <div className="w-full h-full p-4 flex flex-col">
                    ...
                    <div className="absolute inset-0 backdrop-blur-sm bg-white/5 rounded-lg flex items-center justify-center">
                      <div className="text-center z-10">
                        ...
                        <button
                          ...
                          className="w-full px-6 py-3 bg-[#FF660F] text-white text-sm font-semibold rounded-lg hover:bg-[#e55a0a] transition-colors cursor-pointer shadow-lg"
                        >
                          Login / Sign Up
                        </button>
                      </div>
            ...
            <div>
              <button onClick={(e) => { e.stopPropagation(); toggleChatbot(); }} className="w-full flex items-center gap-3 p-[11px] rounded-xl hover:bg-white/10 transition-colors cursor-pointer">
          ...
          <>
            <div className="w-full h-full flex flex-col items-center justify-between py-6">
              ...
              <div>
                <IconOnlyButton onClick={(e) => { if (e) e.stopPropagation(); toggleChatbot(); }} ariaLabel="Back to home">
                  <ArrowLeft size={20} />
                </IconOnlyButton>
              </div>
            </div>
                ...
                <div className="space-y-1">
                  {filteredChats.map((chat) => (
                    <div
                      ...
                      // ADDED: cursor-pointer
                      className={`group w-full rounded-lg py-2 px-3 cursor-pointer transition-colors ${currentSessionId === chat.sessionId ? "bg-white/15" : "hover:bg-white/8"
                        }`}
                    >
                      <p className="text-[14px] font-medium truncate text-white">
                        {chat.title}

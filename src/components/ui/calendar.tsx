...
function Calendar({
  ...
  const to = dpProps.toDate as Date | undefined
    const dayStart = new Date(day.date.getFullYear(), day.date.getMonth(), day.date.getDate()).getTime()
    const outOfRange = (from && dayStart < new Date(from.getFullYear(), from.getMonth(), from.getDate()).getTime()) || (to && dayStart > new Date(to.getFullYear(), to.getMonth(), to.getDate()).getTime())
    ...
    return (
      <Button
        ...
        {...dprops}
      >
        {/* Day content provided by DayPicker */}
        {dprops.children}
        {/* Small availability indicator (hidden when selected) */}
        {!modifiers.selected && (availableInRange || unavailable) && (
          <span
            aria-hidden
            className={cn(
...
function CalendarDayButton({
  ...
  return (
    <Button
      ...
      data-range-middle={modifiers.range_middle}
      className={cn(
    "data-[selected-single=true]:bg-[#343C6A] data-[selected-single=true]:text-white data-[range-middle=true]:bg-accent data-[range-middle=true]:text-accent-foreground data-[range-start=true]:bg-primary data-[range-start=true]:text-primary-foreground data-[range-end=true]:bg-primary data-[range-end=true]:text-primary-foreground group-data-[focused=true]/day:border-ring group-data-[focused=true]/day:ring-ring/50 dark:hover:text-accent-foreground flex aspect-square size-auto w-full min-w-(--cell-size) flex-col gap-1 leading-none font-normal group-data-[focused=true]/day:relative group-data-[focused=true]/day:z-10 group-data-[focused=true]/day:ring-[3px] data-[range-end=true]:rounded-md data-[range-end=true]:rounded-r-md data-[range-middle=true]:rounded-none data-[range-start=true]:rounded-md data-[range-start=true]:rounded-l-md [&>span]:text-xs [&>span]:opacity-70",
    defaultClassNames.day,
    // when disabled (out of range) style - muted text only
    modifiers.disabled && "text-[#ACACAC] pointer-events-none opacity-80",
        className
      )}
      {...props}
    />

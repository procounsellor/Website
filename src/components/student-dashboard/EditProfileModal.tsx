...
const EditProfileModal: React.FC<EditProfileModalProps> = ({ user, isOpen, onClose, onUpdate, requireNameOnly = false }) => {
  ...
  // already phone-OTP, so identity is proven before this modal opens. Verifying
  // a second channel here was losing students mid-flow: whoever didn't finish
  // the email OTP couldn't continue, and coming back meant starting over.
  ...
  // Closing is always allowed. The action that opened this modal does its own
  // checking, and blocking the close only stranded students who wanted out.
  const handleClose = () => {
    onClose();
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    } else if (!firstName.trim() || !email.trim()) {
      ...
      <div className="md:hidden h-full w-full bg-[#F5F5F7] flex flex-col">
        ...
        <div className="grow overflow-y-auto p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-[10px] font-semibold text-[#2F303280] mb-1">
                Email {!requireNameOnly && <span className="text-red-500">*</span>}
              </label>
              <div className="relative h-11">
                <input
                  ...
                  required={!requireNameOnly}
                  className="h-full w-full px-4 bg-white border border-[#EFEFEF] rounded-xl text-base text-gray-800 placeholder-gray-400 focus:ring-2 focus:ring-blue-500"
              ...
              <div className="md:col-span-2">
                ...
                <div className="relative h-12">
                  <input
                    ...
                    required={!requireNameOnly}
                    placeholder="Enter your email"
                    className="w-full h-full px-3 bg-white border border-[#EFEFEF] rounded-xl text-base text-[#718EBF] placeholder-[#718EBF]"
                  />
                </div>
              </div>
            </div>

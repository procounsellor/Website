...
const StatItem = ({ icon, label, value, bgColorClass }: { icon: React.ReactNode, label: string, value: string, bgColorClass: string }) => (
  <div className="flex flex-col items-center text-center">
    ...
    <span className="font-medium text-[#232323]">{value}</span>
    <span className="font-semibold text-[#232323]">{label}</span>
  </div>
);
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
      <div className="flex justify-around items-start">
        <StatItem 
          icon={<Clock className="w-6 h-6 text-[#165DFB]" />} 
          label={examData.duration || '3 Years'} 
          value="Duration" 
          bgColorClass="bg-[#EFF6FF]" 
        />
        <StatItem 
          icon={<Users className="w-6 h-6 text-[#00A63E]" />} 
          label={examData.level || 'National'} 
          value="Level" 
          bgColorClass="bg-[#EFFDF4]"
        />
        <StatItem 
          icon={<Award className="w-6 h-6 text-[#FAA629]" />} 
          label={examData.maxMarks || '300'} 
          value="Max Marks" 
          bgColorClass="bg-[#FFE9C9]"
        />
        <StatItem 
          icon={<Calendar className="w-6 h-6 text-[#E9605A]" />} 

...
async function fetchAll(baseUrl) {
  ...
  const [collegesRaw, coursesRaw, testGroupsRaw, deadlinesRaw] = await Promise.all([
  ...
  return { colleges, courses, testGroups, deadlines, collegeDetails };
}
...
async function writeOutputs(snapshot) {
  const { colleges, courses, testGroups, deadlines, collegeDetails } = snapshot;
